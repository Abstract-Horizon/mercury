...
public class MercurySetup {
    ...
    public void start() throws Exception {
        ...
        File sendqueue = new File(mercuryData, "sendqueue");
        if (!sendqueue.exists()) {
            sendqueue.mkdirs();
        }
        ...
        if (!accountsKeystore.exists()) {
            InputStream is = getClass().getResourceAsStream("/setup/accounts.keystore");
            try {
        ...
        if (!accountsProperties.exists()) {
            InputStream is = getClass().getResourceAsStream("/setup/accounts.properties");
            try {
        ...
        if (!mailSuiteXml.exists()) {
            InputStream is = getClass().getResourceAsStream("/setup/mercury-mail-suite.sar.xml");
            try {
