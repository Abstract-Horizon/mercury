...
public class MaildirFolderData extends Folder {
    ...
    /**
     * Returns folder's del directory
     * @return folder's del directory
     */
    protected File getDelDir() {
        return del;
    }
    ...
    protected boolean deleteAll(File file) {
        ...
        if (res) {
            ...
            res = file_deleted && res;
            if (file_deleted) {
                // TODO deleted dirs should be in the root of mailboxes
                File deleted_dirs = new File(file.getParentFile(), ".deleted_dirs");
                if (deleted_dirs.exists()) {

                }
                // TODO check if exists or not and add file.getName()
            }
        }
        return res;
    }
    ...
    protected void obtainMessages() throws MessagingException {
        try {
            ...
            if ((time - lastAccess) > delay) {
                ...
                if (expunged.size() > 0) {
                    Iterator<MaildirMessage> it = expunged.values().iterator();
                    while (it.hasNext()) {
                        MaildirMessage msg = it.next();
                        data.messages.remove(msg);
                        data.files.remove(msg.getBaseName());
