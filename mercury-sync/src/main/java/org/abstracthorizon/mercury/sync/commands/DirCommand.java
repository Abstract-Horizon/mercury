 ...
 */
public class DirCommand extends SyncCommand {

    protected static final Logger logger = LoggerFactory.getLogger(DirCommand.class);
    ...
    public void execute(SyncSession connection, String command, String cmdLine) throws CommandException, IOException, ParserException {
        try {
            ...
            List<String> response = cachedDirs.toStringList();

            connection.sendResponse(new DirResponse(response.toArray(new String[response.size()])));

            connection.sendResponse(SyncResponses.getCommandReadyResponse("DIR", now));
        } catch (IOException e) {
    ...
    private static class DirResponse extends SyncResponse {
        public DirResponse(String[] msg) {
            super("", msg);
        }
    }
}
